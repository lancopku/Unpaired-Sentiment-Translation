...
'''class Hypothesis(object):
  ...
  def extend(self, token, state, attn_dist=None, p_gen=None, coverage=None, log_prob=None):
...
def run_greedy_decode(sess, model, batch):
    decode_result = model.max_generator(sess,batch)
...
'''def run_greedy_search(sess,model,vocab,batch):
    ...
    while steps < FLAGS.max_dec_steps:
        ...
        ( bestoutput, new_states, p_gens, new_coverage) = model.decode_greedy(sess=sess,
                                                               batch=batch,
        ...
        for i in range(FLAGS.batch_size):
            h, new_state, p_gen, new_coverage_i = hyps[i], new_states[i], p_gens[i], new_coverage[i]
            new_hyp = h.extend(token=bestoutput[0][i],
                               state=new_state,
                               p_gen=p_gen,
                               coverage=new_coverage_i)
            all_best_outputs.append(new_hyp)
        hyps=all_best_outputs
        steps+=1
