...
class BeamSearchDecoder(object):
    ...
    def print_batch(self,batch):
        ...
        tf.logging.info("enc_len")
        tf.logging.info(list(batch.enc_lens))
        tf.logging.info("padding mask")
        tf.logging.info(list(batch.enc_padding_mask))
        ...
        while True:
            ...
            if batch is None:  # finished decoding dataset in single_pass mode
                ...
                tf.logging.info("Output has been saved in %s and %s. Now starting ROUGE eval...", self._rouge_ref_dir,
                                self._rouge_dec_dir)
                gold_set = [[os.path.join(self._rouge_ref_dir, "%06d_reference.txt" % i)] for i in range(counter)]
                pred_set = [os.path.join(self._rouge_dec_dir, "%06d_decoded.txt" % i) for i in range(counter)]
            ...
            best_hyp = beam_search.run_greedy_decode(self._sess, self._model, batch)

            for i in range(FLAGS.batch_size):

                original_review = batch.original_reviews[i]  # string
                output_ids = [int(t) for t in best_hyp[i]['generated'][0]]
                ...
                if FLAGS.single_pass:
                    if 'chinese' in FLAGS.dataset:
                        decoded_words = [data.word2id(x) for x in decoded_words]
                        self.my_write_for_rouge(decoded_words, batch.original_abstract_ids,
                                                counter)  # write ref summary and decoded summary to file, to eval with pyrouge later
                    else:
                        self.my_write_for_rouge(decoded_words, original_review.split(), counter)

                    counter += 1  # this is how many examples we've decoded


    def my_write_for_rouge(self, predict, gold, count):
        ref_file = os.path.join(self._rouge_ref_dir, "%06d_reference.txt" % count)
        decoded_file = os.path.join(self._rouge_dec_dir, "%06d_decoded.txt" % count)
        predictfile = codecs.open(decoded_file, "w", "utf-8")
        for x in predict:
            # for j in x:
            predictfile.write(str(x) + " ")
        predictfile.write('\n')
        predictfile.close()

        goldfile = codecs.open(ref_file, "w", "utf-8")
        for x in gold:
            # for j in x:
            goldfile.write(str(x) + " ")
        goldfile.write('\n')
        goldfile.close()
        tf.logging.info("Wrote example %i to file" % count)
